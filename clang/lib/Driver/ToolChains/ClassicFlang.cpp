...
void ClassicFlang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  */
  ...
  DiagnosticsEngine DE(DiagID, DiagOpts, new IgnoringDiagConsumer());
  std::unique_ptr<clang::TargetOptions> TO = std::make_unique<clang::TargetOptions>();
  TO->Triple = Triple.getTriple();
  std::unique_ptr<TargetInfo> TI(clang::TargetInfo::CreateTargetInfo(DE, *TO));
  ...
  LowerCmdArgs.push_back("-x"); LowerCmdArgs.push_back("249"); LowerCmdArgs.push_back("150"); // LLVM version
