...
def generate_buildoptions(arguments):
  ...
  if arguments.c_compiler:
    base_cmake_args.append(f'-DCMAKE_C_COMPILER={arguments.c_compiler}')

  if arguments.cxx_compiler:
    base_cmake_args.append(f'-DCMAKE_CXX_COMPILER={arguments.cxx_compiler}')
...
def configure_llvm(arguments):
  ...
  if(arguments.llvm_enable_projects):
    additional_options.append('-DLLVM_ENABLE_PROJECTS='+'; '.join(['%s']*len(arguments.llvm_enable_projects)) % tuple(arguments.llvm_enable_projects))
